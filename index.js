...
class BlockChain {
    ...
    checkChainValidity() {
        for (let i = 1; i < this.blockchain.length; i++) {
            ...
            if (currentBlock.predcedingHash !== precedingBlock.hash()) {
                return false
            }
        }
        return true
...
class BassCoin {
    ...
    validateNewChain = (req, res, next) => {
        if (req.body) {
            if (req.body.id &&
                ...
                res.status(400).json({ message: 'requuest format invalid' })
            }
        } else {
            res.status(400).json({ message: 'request format invalid' })
        }
    }
    createNewChain = (req, res) => {
        ...
        res.status(200).json({ message: 'chain created', data: block })
    }
...
app.listen(3000, () => {
    console.log('blockchain running on 3000')
})